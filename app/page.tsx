...
import { Github, Linkedin, Mail, Trophy, Globe, Atom, Calendar } from 'lucide-react';
      ...
      <main className="max-w-5xl mx-auto px-6 py-12">
        ...
        <section className="mb-20">
          <h1 className="text-4xl md:text-6xl font-bold mb-6 tracking-tight">
            Hey there!, I'm <span className="text-black bg-gradient-to-r from-green-400/20 to-green-200/20 px-2 rounded-lg">[Name]</span>
          </h1>
          <p className="text-xl text-gray-600 leading-relaxed max-w-2xl mb-8">
            ...
            Currently specializing in Frontend (React / Next.js) and Full-stack Engineering.
          </p>
        ...
        <section className="mb-24">
          ...
          <div className="flex flex-col md:flex-row gap-12 items-start">
            ...
            <div className="flex-1 space-y-6 text-lg text-gray-600 leading-relaxed">
              <p>
                Hi Software Engineer. A self-taught developer with an interest in Computer Science. 
                Currently specializing in Frontend (React / Next.js) in an environment of Full-stack Engineering.
              </p>
              <p>
                I experience with <span className="text-green-600 font-medium">no-code tools</span> and <span className="text-green-600 font-medium">open source</span>. 
                Currently experienced with no-code tools and communications, sample experience, software and development.
              </p>
              <p>
                I think and type to experience in interest with <span className="text-green-600 font-medium underline underline-offset-4 decoration-green-300">open source</span> and 
                open sources are incredibly customizable interests for extend or projects of software logic.
              </p>
            </div>
          </div>
        </section>
...
function SocialButton({ icon, label }: { icon: React.ReactNode, label: string }) {
  return (
    <button className="flex items-center gap-2 px-4 py-2 border rounded-lg hover:bg-gray-100 hover:scale-105 active:scale-95 transition-all duration-200 text-sm font-medium group">
       <span className="group-hover:text-green-600 transition-colors">{icon}</span> {label}
    </button>
  );
}
    ...
    return (
        <div className="group h-full flex flex-col rounded-2xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:border-green-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
            ...
            <div className="p-6 flex flex-col flex-1">
                <h3 className="text-xl font-bold mb-2 group-hover:text-green-700 transition-colors">{title}</h3>
                <p className="text-gray-600 mb-6 flex-1 text-sm leading-relaxed">{desc}</p>
                
                <div className="flex flex-wrap gap-2 mb-6">
                    {tags.map((tag) => (
                        <span key={tag} className="px-2 py-1 bg-white border border-gray-200 rounded-md text-xs font-medium text-gray-500">
                            {tag}
                        </span>
                    ))}
                </div>

                <a href={link} className="mt-auto w-full py-2.5 px-4 bg-white border border-gray-200 text-gray-700 font-medium rounded-xl hover:bg-green-600 hover:text-white hover:border-green-600 transition-all duration-300 text-center flex items-center justify-center gap-2 text-sm shadow-sm group-hover:shadow-md">
...
function ExperienceItem({ date, company, title, desc, isLast = false }: any) {
  return (
    <div className="flex gap-8 group">
      <div className="w-24 pt-1 text-sm text-gray-400 shrink-0 font-mono">{date}</div>
      <div className="relative pb-12 flex flex-col items-start">
        {!isLast && <div className="absolute left-[7px] top-6 bottom-0 w-[1px] bg-gray-200 group-hover:bg-gray-300 transition-colors" />}
        ...
        <div className="pl-8 transition-transform group-hover:translate-x-1 duration-200">
          ...
          <p className="text-gray-500 text-sm max-w-lg leading-relaxed">{desc}</p>
        </div>
      </div>
    </div>
  );
